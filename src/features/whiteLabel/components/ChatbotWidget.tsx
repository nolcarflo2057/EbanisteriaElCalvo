...
export function ChatbotWidget({ config }: { config: ConfigData }) {
  ...
  useEffect(() => {
    if (open && config?.chatbotGreeting) {
      const greetingMessage: Message = { role: "assistant", content: config.chatbotGreeting };
      setMessages([greetingMessage]);
    }
  }, [open, config?.chatbotGreeting]);
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-lg bg-white/80 backdrop-blur-md shadow-xl">
            ...
            <div className="p-4 overflow-y-auto max-h-[70vh]">
              ...
              {streaming && (
                <div className="text-left">
                  <span className="inline-block p-2 rounded-lg bg-gray-200 text-gray-800">Pensando… escribiendo</span>
                </div>
              )}
            </div>
            
            {/* Quick Reply Tags */}
            {!streaming && config.chatbotTags && config.chatbotTags.length > 0 && (
            ...
            <div className="border-t p-2">
              ...
              />
              <Button onClick={handleSend} disabled={streaming || !input.trim()} className="w-full">
