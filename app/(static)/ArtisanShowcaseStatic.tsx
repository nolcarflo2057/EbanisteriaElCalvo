...
import type { ArtisanShowcaseBlockProps } from "@/features/blocks/blocks/artisan-showcase/artisan-showcase.schema";
import { shouldUnoptimize } from "@/shared/utils/image";

export function ArtisanShowcaseStatic(props: ArtisanShowcaseBlockProps) {
			...
			<div className="max-w-[1400px] mx-auto px-4 md:px-10">
				<div className={cn("grid md:grid-cols-2 gap-16 items-stretch", isImageLeft ? "" : "md:flex-row-reverse")}>
					<div className="relative h-full min-h-[400px] group overflow-hidden rounded-2xl">
						<div className="h-full w-full rounded-2xl overflow-hidden shadow-2xl relative z-10 transition-transform duration-700 ease-out group-hover:scale-105">
							<Image
								...
								unoptimized={shouldUnoptimize(props.image)}
							/>
						</div>
					...
					<div className="flex flex-col justify-center">
						{props.badge && (
							<span className="text-secondary font-label-md uppercase tracking-widest block mb-4">{props.badge}</span>
						)}
						<h2 className="font-headline-md text-headline-md mb-6 text-primary">{props.title}</h2>
