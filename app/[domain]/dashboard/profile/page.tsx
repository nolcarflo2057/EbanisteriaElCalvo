...
export default function ProfilePage() {
	...
	// Sync state and load store settings
	useEffect(() => {
		if (session?.user) {
	...
	return (
		<div className="flex flex-col w-full max-w-3xl mx-auto px-5 space-y-8 animate-in fade-in duration-300">
			<div>
			...
			<div className="bg-card rounded-2xl shadow-xl border border-border overflow-hidden">
				...
				<div className="relative flex flex-col items-center -mt-28 pb-6 px-6">
					...
					<p className="text-xs capitalize flex items-center gap-1.5 mt-1 px-3 py-1 rounded-full border border-primary/25 bg-primary/10 text-primary font-semibold shadow-sm animate-in fade-in duration-300">
			...
			<Dialog
				isOpen={isUploadOpen}
				onClose={() => !isUploadingImage && setIsUploadOpen(false)}
			...
			>
				{isUploadingImage ? (
					<div className="flex flex-col items-center justify-center p-10 space-y-4">
						...
						<p className="text-sm font-medium text-foreground">Subiendo tu foto...</p>
						<p className="text-xs text-muted-foreground text-center">
							Por favor, espera un momento mientras transferimos y procesamos la imagen.
						</p>
					</div>
				) : (
